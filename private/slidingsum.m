...
%  of N larger or equal to 2*(nx - 1), each value of the output data array are identical and equal to sum(in).
%
...
nx   = length(in);             % The length of the input data structure is acquired to later evaluate the 'sum' over the appropriate boundaries.

if (N>=(2*(nx-1)))                                                     % If the number of neighbouring points over which the sliding 
 out = sum(in)*ones(size(in));                                        % average will be performed is large enough, then the average actually covers all the points
...
for i=1:nx,                                                 % For each element (i-th) contained in the input numerical array, a check must be performed:
 if ((i-m) < 1) & ((i+m) <= nx)                             % If not enough points are available on the left of the i-th element..
  out(i) = sum(in(1:i+m));                                 % then we proceed to evaluate the sum from the first element to the (i + m)-th.
 elseif ((i-m) >= 1) & ((i+m) <= nx)                        % If enough points are available on the left and on the right of the i-th element..
  out(i) = sum(in(i-m:i+m));                               % then we proceed to evaluate the sum on 2*m elements centered on the i-th position.
elseif ((i-m) >= 1) & ((i+m) > nx)                          % If not enough points are available on the rigth of the i-th element..
  out(i) = sum(in(i-m:nx));                                % then we proceed to evaluate the sum from the element (i - m)-th to the last one.
 elseif ((i-m) < 1) & ((i+m) > nx)                          % If not enough points are available on the left and on the rigth of the i-th element..
  out(i) = sum(in(1:nx));                                  % then we proceed to evaluate the sum from the first element to the last.
 end % if
end % for i
