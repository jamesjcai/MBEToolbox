...
/*
      ...
      mexPrintf("     use as %s already exists.\n", filedesc);
      mexPrintf("     Do you want to Replace it, Append to it,\n");
      mexPrintf("     write to a new File, or Quit?\n");
      loopcount2 = 0;
      do {
        mexPrintf("     (please type R, A, F, or Q) \n");
#ifdef WIN32
        phyFillScreenColor();
#endif
...
*/
      ...
      if (ch == 'A') {
        ...
        loopcount2 = 0;
        while (file[0] =='\0') {
          mexPrintf("Please enter a new file name> ");
          getstryng(file);
          countup(&loopcount2, 10);
		}
        strcpy(filemode,"w");
    ...
    if (of)
      ...
      case 'a':
        ...
        continue;
      default:
     mexPrintf("There is some error in the call of openfile. Unknown mode.\n");
        exxit(-1);
      }
    }
...
#else
  mexPrintf("%s", ((ibmpc || ansi) ? ("\033[2J\033[H") : "\n\n"));
#endif
} /* cleerhome */
...
{ /* input random number seed */
  ...
  loopcount = 0;
  do {
    mexPrintf("Random number seed (must be odd)?\n");
    scanf("%ld%*[^\n]", inseed);
    getchar();
    countup(&loopcount, 10);
...
{ /* input number of jumblings for jumble option */
  ...
  loopcount = 0;
  do {
    mexPrintf("Number of times to jumble?\n");
    scanf("%ld%*[^\n]", njumble);
    getchar();
    countup(&loopcount, 10);
  } while ((*njumble) < 1);
...
{ /* input outgroup number */
  ...
  loopcount = 0;
  do {
    mexPrintf("Type number of the outgroup:\n");
    scanf("%ld%*[^\n]", outgrno);
    getchar();
    done = (*outgrno >= 1 && *outgrno <= spp);
    if (!done) {
      mexPrintf("BAD OUTGROUP NUMBER: %ld\n", *outgrno);
      mexPrintf("  Must be in range 1 - %ld\n", spp);
    }
    countup(&loopcount, 10);
  } while (done != true);
}  /*initoutgroup*/
...
{ /* input threshold for threshold parsimony option */
  ...
  loopcount = 0;
  do {
    mexPrintf("What will be the threshold value?\n");
    scanf("%lf%*[^\n]", threshold);
    getchar();
    done = (*threshold >= 1.0);
    if (!done)
      mexPrintf("BAD THRESHOLD VALUE:  it must be greater than 1\n");
    else
...
{ /* initialize category number for rate categories */
  ...
  *categs = 0;
  do {
    mexPrintf("Number of categories (1-%d)?\n", maxcategs);
    scanf("%ld%*[^\n]", categs);
    getchar();
    countup(&loopcount, 10);
...
{ /* initialize category rates for HMM rates */
  ...
  loopcount = 0;
  for (;;){
    mexPrintf("Rate for each category? (use a space to separate)\n");
    getstryng(line);
    done = true;
    for (i = 0; i < categs; i++){
     ...
     mexPrintf("Please enter exactly %ld values.\n",categs);
     done = false;
     break;
      }
      strcpy(line,rest);
...
{ /* input probabilities of rate categores for HMM rates */
  ...
  loopcount = 0;
  do {
    mexPrintf("Probability for each category?");
    mexPrintf(" (use a space to separate)\n");
    getstryng(line);
    done = true;
    for (i = 0; i < categs; i++){
      scanned = sscanf(line,"%lf %[^\n]",&probcat[i],rest);
    ...
    *probsum = 0.0;
    ...
    if (fabs(1.0 - (*probsum)) > 0.001) {
      ...
      mexPrintf("Probabilities must add up to");
      mexPrintf(" 1.0, plus or minus 0.001.\n");
    }
    countup(&loopcount, 100);
  } while (!done);
}  /*initprobcat*/
...
{/* input how many cycles */
  ...
  loopcount = 0;
  do { 
    mexPrintf("How many cycles of %4ld trees?\n", howoften);
    scanf("%ld%*[^\n]", howmanny);
    getchar();
    countup(&loopcount, 10);
  } while (*howmanny <= 0);
...
{ /* input how many trees per cycle */
  ...
  loopcount = 0;
  do {
    mexPrintf("How many trees per cycle?\n");
    scanf("%ld%*[^\n]", howoften);
    getchar();
    countup(&loopcount, 10);
  } while (*howoften <= 0);
...
{ /* input patch length parameter for autocorrelated HMM rates */
  ...
  loopcount = 0;
  do {
    mexPrintf("Mean block length of sites having the same rate (greater than 1)?\n");
    scanf("%lf%*[^\n]", lambda);
    getchar();
    countup(&loopcount, 10);
  } while (*lambda <= 1.0);
...
{ /* input frequencies of the four bases */
  ...
  mexPrintf("Base frequencies for A, C, G, T/U (use blanks to separate)?\n");
  loopcount = 0;
  do {
    getstryng(input);
    ...
    if (scanned == 4)
      ...
      mexPrintf("Please enter exactly 4 values.\n");
    countup(&loopcount, 100);
  } while (1);
}  /* initfreqs */
...
{ /* input transition/transversion ratio */
  ...
  loopcount = 0;
  do {
    mexPrintf("Transition/transversion ratio?\n");
    scanf("%lf%*[^\n]", ttratio);
    getchar();
    countup(&loopcount, 10);
  } while (*ttratio < 0.0);
...
void initdatasets(long *datasets)
{
  ...
  loopcount = 0;
  do {
    mexPrintf("How many data sets?\n");
    scanf("%ld%*[^\n]", datasets);
    getchar();
    done = (*datasets > 1);
      if (!done)
     mexPrintf("Bad data sets number:  it must be greater than 1\n");
    countup(&loopcount, 10);
  } while (!done);
} /* initdatasets */
...
void justweights(long *datasets)
{
  ...
  loopcount = 0;
  do {
    mexPrintf("How many sets of weights?\n");
    scanf("%ld%*[^\n]", datasets);
    getchar();
    done = (*datasets >= 1);
      if (!done)
     mexPrintf("BAD NUMBER:  it must be greater than 1\n");
    countup(&loopcount, 10);
  } while (!done);
} /* justweights */
...
void inputnumbers2(long *spp, long *nonodes, long n)
{
  ...
  if (fscanf(infile, "%ld", spp) != 1 || *spp <= 0) {
    mexPrintf("ERROR: Unable to read the number of species in data set\n");
    mexPrintf(
      "The input file is incorrect (perhaps it was not saved text only).\n");
  }
  fprintf(outfile, "\n%4ld Populations\n", *spp);
  ...
  while (!(eoln(infile))) {
    ...
    if (strchr(options, ch) != NULL)
     (* extranum)++;
      ...
      mexPrintf("BAD OPTION CHARACTER: %c\n", ch);
      exxit(-1);
    }
  }
  scan_eoln(infile);
...
{  /* match option characters to those in auxiliary options line */
  ...
  *ch = gettc(infile);
  uppercase(ch);
  if (strchr(options, *ch) == NULL) {
    mexPrintf("ERROR: Incorrect auxiliary options line");
    mexPrintf(" which starts with %c\n", *ch);
    exxit(-1);
  }
}  /* matchoptions */
  ...
  *weightsum = 0;
  for (i = a; i < b; i++) {
    ...
    if (ch == '0' || ch == '1')
      ...
      mexPrintf("weights in %s must be 0 or 1\n", prog);
      exxit(-1);
    }
    *weightsum += weight[i];
  }
...
void inputcategs(long a, long b, steptr category, long categs,const char *prog)
{
  ...
  for (i = a; i < b; i++) {
    ...
    if ((ch >= '1') && (ch <= ('0'+categs)))
      ...
      mexPrintf(" -- categories in %s are currently 1-%ld\n", prog, categs);
      exxit(-1);
    }
  }
  scan_eoln(catfile);
...
void initname(long i)
{
  ...
  for (j = 0; j < nmlngth; j++) {
    if (eoff(infile) | eoln(infile)){
      ...
      mexPrintf(" in the middle of species name for species %ld\n\n", i+1);
      exxit(-1);
    }
    ...
    if ((nayme[i][j] == '(') || (nayme[i][j] == ')') || (nayme[i][j] == ':')
      ...
      mexPrintf("       In name of species number %ld there is character %c\n\n",
              i+1, nayme[i][j]);
      exxit(-1);
    }
  }
  ...
  while (!done) {
    if (c == ',') {
      ...
      if (*ch == '(' || *ch == ',' || *ch == ';') {
        ...
        mexPrintf("unmatched parenthesis or non-bifurcated node\n\n");
        exxit(-1);
      } else {
        if (*ch == ')')
          done = true;
      ...
      if (*ch != ';') {
        mexPrintf("\n\nERROR in user tree %ld: ", which);
        mexPrintf("unmatched parenthesis or missing semicolon\n\n");
        exxit(-1);
      } else
        done = true;
    }
...
{ /* skip forward in user tree until find character c */
  ...
  while (!done) {
    if (c == ',') {
      if (*ch == '(' || *ch == ')' || *ch == ':' || *ch == ';') {
        mexPrintf("\n\nERROR in user tree: ");
        mexPrintf("unmatched parenthesis, missing comma");
        mexPrintf(" or non-trifurcated base\n\n");
     exxit(-1);
      } else if (*ch == ',')
        done = true;
    } else if (c == ')') {
...
{ /* error message if attempt to malloc too little or too much memory */
  mexPrintf ("ERROR: a function asked for an inappropriate amount of memory:");
  mexPrintf ("  %ld bytes\n", x);
  mexPrintf ("       This can mean one of two things:\n");
  mexPrintf ("       1.  The input file is incorrect");
  mexPrintf (" (perhaps it was not saved as Text Only),\n");
  mexPrintf ("       2.  There is a bug in the program.\n");
  mexPrintf ("       Please check your input file carefully.\n");
  mexPrintf ("       If it seems to be a bug, please mail joe@gs.washington.edu\n");
  mexPrintf ("       with the name of the program, your computer system type,\n");
  mexPrintf ("       a full description of the problem, and with the input data file.\n");
  mexPrintf ("       (which should be in the body of the message, not as an Attachment).\n");
...
{ /* Count the number of nodes in a ring, return the total number of */
  ...
  if (p->tip) {
   mexPrintf ("Error: the function count_sibs called on a tip.  This is a bug.\n");
    exxit (-1);
  }
  ...
  while (q != p) {
    if (q == NULL) {
      mexPrintf ("Error: a loop of nodes was not closed.\n");
      exxit (-1);
    } else {
      return_int++;
      q = q->next;
        ...
        initptr initnode,boolean unifok, long maxnodes)
{
  ...
  if ((*ch) == '(') {
    ...
    if ( maxnodes != -1 && nodei > maxnodes) {
      mexPrintf("ERROR in input tree file: Attempting to allocate too\n");
      mexPrintf("many nodes. This is usually caused by a unifurcation.");
      mexPrintf("To use this tree with this program  use Retree to read\n");
      mexPrintf("and write this tree.\n");
      exxit(-1);
    }
    ...
    if ( furs <= 1 && !unifok ) {
      mexPrintf("ERROR in input tree file: A Unifurcation was detetected.\n");
      mexPrintf("To use this tree with this program use retree to read and");
      mexPrintf(" write this tree\n");
      exxit(-1);
    }
        ...
        boolean unifok,long maxnodes)
{
  ...
  if ((*ch) == '(') {
    ...
    if ( maxnodes != -1 && current_loop_index > maxnodes) {
      mexPrintf("ERROR in intree file: Attempting to allocate too many nodes\n");
      mexPrintf("This is usually caused by a unifurcation.  To use this\n");
      mexPrintf("intree with this program  use retree to read and write\n"); 
      mexPrintf("this tree.\n");
      exxit(-1);
    }
    ...
    if ( furs <= 1 && !unifok ) {
      mexPrintf("ERROR in intree file: A Unifurcation was detetected.\n");
      mexPrintf("To use this intree with this program use retree to read and");
      mexPrintf(" write this tree\n");
      exxit(-1);
    }
  ...
  if ((*ch) == '[') {    /* getting tree weight from last comment field */
    ...
    (*trweight) = 1.0 ;
    if (!eoln(treefile))
      mexPrintf("WARNING: tree weight set to 1.0\n");
  }
  else
