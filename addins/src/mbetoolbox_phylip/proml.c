...
void usage(void)
{
    ...
    fprintf(stderr,"  -m<number>  1=Jones-Taylor-Thornton model, 2=Dayhoff PAM model\n");
...
{  /* Protein Maximum Likelihood */
  ...
  printf("---------------------------------------\n");
  printf("  MBETOOLBOX_PROML\n");
  printf("  The modified version of PROML\n");
  ...
  outgropt = false;
      ...
      while ((argc > 1) && (argv[1][0] == '-')) {
        switch (argv[1][1]) {
				...
				switch (tempint){
					case 1:
						usejtt=true; usepam=false;
					printf ("select 1\n");
					break;
				case 2:
					usejtt=false;		usepam=true;
									printf ("select 2\n");
					break;
				default :
					usejtt=true;		usepam=false;
