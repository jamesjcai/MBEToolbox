...
void usage(void)
{
    ...
    fprintf(stderr,"  -f<0|1>     Use empirical base frequencies\n");
    fprintf(stderr,"   -A<0.25>    freqa\n");	
    fprintf(stderr,"   -C<0.25>    freqc\n");	
    fprintf(stderr,"   -G<0.25>    freqg\n");	
    fprintf(stderr,"   -T<0.25>    freqt\n");	
	fprintf(stderr,"  -r<1|2|3>   Rate variation among sites: 1=Constant, 2=Gamma, 3=Gamma+Invariant\n");
	fprintf(stderr,"   -p<1.0>     alpha for Gamma distribution\n");
	fprintf(stderr,"   -c<1-9>     Number of categories (1-9)\n");
	fprintf(stderr,"   -i<0.25>    Fraction of invariant sites?\n");
	fprintf(stderr,"   -a<number>  Rates at adjacent sites correlated, <number>=mean block length\n");
	fprintf(stderr,"  -s<0|1>     Speedier but rougher analysis?\n");
	fprintf(stderr,"  -g<0|1>     Global rearrangements?\n");
	fprintf(stderr,"  -j<number>  Number of times to jumble (0=Not randomize input order)\n");
	fprintf(stderr,"  -z<number>  Seed\n");
...
{  /* DNA Maximum Likelihood */
  ...
  printf("---------------------------------------\n");
  printf("  MBETOOLBOX_DNAML\n");
  printf("  The modified version of DNAML\n");
  ...
  ttr = false;
    ...
    while ((argc > 1) && (argv[1][0] == '-')) {
        switch (argv[1][1]) {
            case 't':								
				ttr=true; ttratio = atoi(&argv[1][2]);
                break;
            case 'f':
            ...
            case 'T':
				freqt = atof(&argv[1][2]);
                break;
            case 'r':	 					// Rate variation among sites: 1=Constant rates, 2=Gamma, 3=Gamma+Invariant
                tempint = atoi(&argv[1][2]);
				switch (tempint)
				{
				case 1:
					rctgry=false; gama=false; invar=false;
				break;
				case 2:
				rctgry=true; gama=true; invar=false;
				break;
				case 3:
				rctgry=true; gama=false; invar=true;
				break;
				default:
					rctgry=false; gama=false; invar=false;
				}
				break;
			case 'p':
				tempint = atof(&argv[1][2]);
				alpha = 1.0 / (tempint * tempint);
				break;
			case 'c':
				rcategs = atoi(&argv[1][2]);
				break;
			case 'i':
				invarfrac = atof(&argv[1][2]);
				break;
             case 'a':								// -a<number>	Rates at adjacent sites correlated,	 <number>=mean block length
                tempint = atoi(&argv[1][2]);
				if(tempint==0){
					auto_ = true;
				}else{
					if (tempint<=1.0){	exit (8);};
					auto_ = false;
					lambda = 1.0 / tempint;
				};	
                break;
            case 's':
				...
				if(tempint==0){
					...
					improve = false;		// use speedier analysis, i.e., not improve
				};				
                break;
                case 'g':
                tempint = atoi(&argv[1][2]);
